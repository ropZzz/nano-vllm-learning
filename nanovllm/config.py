...
class Config:
    model: str  # 模型目录或名称
    max_num_batched_tokens: int = 16384  # 单批次最大token数     所有batch总的tokens数
    max_num_seqs: int = 512  # 单批次最大序列数     单个批次能最多拼512batch
    max_model_len: int = 4096  # 单序列最大长度     window chunk
