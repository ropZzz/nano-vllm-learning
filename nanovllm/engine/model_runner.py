...
class ModelRunner:
    ...
    def allocate_kv_cache(self):
        ...
        config.num_kvcache_blocks = int(total * config.gpu_memory_utilization - used - peak + current) // block_bytes  # 即 num_pages
        ...
        # kv cache 维度： 2(k和v)， 层数，块数，块大小，头数，头维度
        layer_id = 0
    ...
    def prepare_block_tables(self, seqs: list[Sequence]):
        """
        构造并返回补齐长度后的block_table张量。
        
        该方法遍历给定的序列列表，找出最长的block_table长度，将所有序列的
        block_table用-1填充至相同长度，然后将其转换为具有固定内存且异步传输至
        CUDA设备的PyTorch张量。
        
        Args:
            seqs (list[Sequence]): 序列对象列表，每个序列需包含block_table属性。
        
        Returns:
            torch.Tensor: 补齐并转换为CUDA张量的block_table，数据类型为torch.int32。
        """
