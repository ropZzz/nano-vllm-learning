    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor):
        ...
        if context.is_prefill:
            ...
            o = flash_attn_varlen_func(q, k, v,
                                       max_seqlen_q=context.max_seqlen_q,   # batch 中单序列最大 query 长度
                                       cu_seqlens_q=context.cu_seqlens_q,   # cumsum seqlen q
                                       max_seqlen_k=context.max_seqlen_k,   # batch 中单序列最大 key 长度
                                       cu_seqlens_k=context.cu_seqlens_k,   # cumsum seqlen k
                                       softmax_scale=self.scale,            # flashscale
                                       causal=True,                         # 启用下三角因果掩码
